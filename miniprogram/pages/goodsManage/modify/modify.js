...
Page({
  ...
  onLoad: function (e) {
      ...
      this.setData({
          ...
          detail:e.detail,
          intro:e.intro
          //InputList[0].placeholder:e.goodsname,
      })
      console.log(e)
      console.log("id",this.data.id)
      console.log("goodsname",this.data.goodsname)
  ...
  Submit(e) {
      ...
      let FormData2 = this.data.FormData
      let InputList = this.data.InputList
      
      // 表单数据的校验
     
        if(FormData2.goodsname == ''){
          FormData2.goodsname = this.data.goodsname
        }
        if(FormData2.price == ''){
          FormData2.price = this.data.price
        }
        if(FormData2.intro == ''){
          FormData2.intro = this.data.intro
          console.log("空",this.data.intro)
        }
        if(FormData2.detail == ''){
          FormData2.detail = this.data.detail
          console.log("空2",this.data.detail)
        }
          ...
          wx.cloud.uploadFile({
              ...
              success(res) {
                  ...
                  if (imgPathList.length == that.data.imgList.length) {
                      // 保存信息
                      that.SubmitGoodsImg(imgPathList)
                  }
              },
              fail: err => {
                  wx.hideLoading()
